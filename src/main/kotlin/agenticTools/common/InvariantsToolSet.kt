...
class InvariantsToolSet(
    ...
    @LLMDescription(
        """
            ...
            If verification error is not null and from this error you can conclude, for example, that some invariant or postcondition cannot be proven - 
    ...
    @LLMDescription(
        """
            ...
            It can have verification error set to null (if the agent just started solving task) or set to some error, obtained from verifier.
            If verification error is not null and from this error you can conclude, for example, that some invariant cannot be proven, 
...
//    It can simplify some invariant, eliminate unnecessary/incorrect parts of it, add trigger inside the forall invariant (that will simplify proof and help to avoid timeout when proving) or rewrite the trigger.
//    Couple of examples of errors and code snippets for which this tools is eligible:
//
//    1.
//"""
//        File("a").readText()
